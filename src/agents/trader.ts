...
export class TraderAgent extends BaseAgent<TraderInput, TradeProposal> {
  ...
  public async run(input: TraderInput, ctx: AgentContext): Promise<TradeProposal> {
    const side = input.debate.final_bias === "bearish" ? "short" : "long";
    const atr = input.analysts.technical.indicators.atr || input.lastPrice * 0.01;
    ...
    const conviction = (input.analysts.technical.confidence + input.debate.confidence) / 2;
    ...
    const output: TradeProposal = {
      ...
      position_size_pct,
      timeframe: "1h",
      reasoning: `bias=${input.debate.final_bias}; tech=${input.analysts.technical.signal}; sentiment=${input.analysts.sentiment.mood}`
    };
